...
/*
* File: HashTableLin.cpp
* Author: Rahim Aziz
* Course: 2SI3 Lab 5
*
* Summary of File:
* In this lab you will implement a hash table with linear probing and perform simulations to observe its performance.
...
void HashTableLin::insert(int n)
{
	...
	if (isIn(n) == false) {
		// Calculate the first hash function value and initialize the linear probing index
		int hash_function = n % getTableSize();
		int probe_index = 0;

		// If we have reached a non-zero digit continue the linear probing process
		while (this->table[hash_function] != 0) {
			// Increment the probe index and recalculate the hash function using linear probing
			probe_index++;
...
bool HashTableLin::isIn(int n)
{
	...
	if (getTableSize() > 0) {
		...
		// If we haven't reached an empty hash table slot continue linear probing
		while (this->table[hash_function] != 0) {
...
/*
* HashTableLin::insertCount(int n)
*
...
int HashTableLin::insertCount(int n) {
	...
	// If the desired position is not found commence linear probing until a position is found
	while (this->table[hash_function] != 0) {
		probe_index++;
		hash_function = (n + probe_index) % getTableSize();
	}
...
*
* Description: Performs simulation to measure the average number of probes for successful search for linear probing
*/
std::vector<double> HashTableLin::simProbeSuccess()
{
	...
	for (double i = 0.1; i < 1; i += 0.1) {
		...
		for (int j = 0; j < 100; j++) {
			...
			HashTableLin sim_prob_table(10000, i);
			...
			// Randomly generate a number and insert it into the hash table using linear probing
